...
def main():
    ...
    agent_type = (
        ...
        input_data.get("subagent", "") or
        input_data.get("name", "")
    )

    last_assistant_message = (
        input_data.get("last_assistant_message", "") or
    ...
    if verify_commands:
        ...
        if passed:
            ...
            output = {
                "decision": "allow",
                "reason": "All verify commands passed.",
            }
            print(json.dumps(output, ensure_ascii=False))
            sys.exit(0)
        else:
        ...
        if not missing:
            ...
            output = {
                "decision": "block",
                "reason": f"Iteration {current_iteration}/{MAX_ITERATIONS}. Missing: {', '.join(missing)}",
            }
            print(json.dumps(output, ensure_ascii=False))
            sys.exit(0)
