...
```bash
cat bwflow/spec/<category>/<file>.md
```
...
```
Development Flow:
  Learn something → /bwflow:update-spec → Knowledge captured
       ↑                                  ↓
  /bwflow:break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/bwflow:break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/bwflow:update-spec` - Actually makes the updates (this command)
- `/bwflow:finish-work` - Reminds you to check if specs need updates
