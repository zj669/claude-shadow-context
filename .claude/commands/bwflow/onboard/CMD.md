...
**The Solution**: The `bwflow/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `/bwflow:start` command reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `/check-*` commands re-verify code against guidelines AFTER writing, catching drift that occurred during development. The `/bwflow:finish-work` command does a final holistic review.
...
### /bwflow:start - Restore AI Memory
...
**WHY THIS MATTERS**:
- Without /bwflow:start: AI is blind. It might work on wrong branch, conflict with others' work, or redo already-completed work.
- With /bwflow:start: AI knows project context, can continue where previous session left off, avoids conflicts.

---

### /bwflow:before-frontend-dev and /bwflow:before-backend-dev - Inject Specialized Knowledge

**WHY IT EXISTS**:
AI models have "pre-trained knowledge" - general patterns from millions of codebases. But YOUR project has specific conventions that differ from generic patterns.

**WHAT IT ACTUALLY DOES**:
1. Reads `bwflow/spec/frontend/` or `bwflow/spec/backend/`
...
### /bwflow:check-frontend and /bwflow:check-backend - Combat Context Drift
...
### /bwflow:check-cross-layer - Multi-Dimension Verification
...
### /bwflow:finish-work - Holistic Pre-Commit Review
...
### /bwflow:record-session - Persist Memory for Future

**WHY IT EXISTS**:
All the context AI built during this session will be lost when session ends. The next session's `/bwflow:start` needs this information.
...
**[1/8] /bwflow:start** - AI needs project context before touching code
**[2/8] python3 ./bwflow/scripts/task.py create "Fix bug" --slug fix-bug** - Track work for future reference
**[3/8] /bwflow:before-frontend-dev** - Inject project-specific frontend knowledge
**[4/8] Investigate and fix the bug** - Actual development work
**[5/8] /bwflow:check-frontend** - Re-verify code against guidelines
**[6/8] /bwflow:finish-work** - Holistic cross-layer review
**[7/8] Human tests and commits** - Human validates before code enters repo
**[8/8] /bwflow:record-session** - Persist memory for future sessions

### Example 2: Planning Session (No Code)

**[1/4] /bwflow:start** - Context needed even for non-coding work
...
**[4/4] /bwflow:record-session (with --summary)** - Planning decisions must be recorded

### Example 3: Code Review Fixes

**[1/6] /bwflow:start** - Resume context from previous session
**[2/6] /bwflow:before-backend-dev** - Re-inject guidelines before fixes
**[3/6] Fix each CR issue** - Address feedback with guidelines in context
**[4/6] /bwflow:check-backend** - Verify fixes didn't introduce new issues
**[5/6] /bwflow:finish-work** - Document lessons from CR
**[6/6] Human commits, then /bwflow:record-session** - Preserve CR lessons

### Example 4: Large Refactoring

**[1/5] /bwflow:start** - Clear baseline before major changes
**[2/5] Plan phases** - Break into verifiable chunks
**[3/5] Execute phase by phase with /check-* after each** - Incremental verification
**[4/5] /bwflow:finish-work** - Check if new patterns should be documented
**[5/5] Record with multiple commit hashes** - Link all commits to one feature

### Example 5: Debug Session

**[1/6] /bwflow:start** - See if this bug was investigated before
**[2/6] /bwflow:before-backend-dev** - Guidelines might document known gotchas
**[3/6] Investigation** - Actual debugging work
**[4/6] /bwflow:check-backend** - Verify debug changes don't break other things
**[5/6] /bwflow:finish-work** - Debug findings might need documentation
**[6/6] Human commits, then /bwflow:record-session** - Debug knowledge is valuable
...
4. **Record everything** - /bwflow:record-session persists memory.
...
1. Run `/bwflow:record-session` to record this onboard session
