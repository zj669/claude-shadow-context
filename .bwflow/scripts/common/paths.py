#!/usr/bin/env python3
"""
Common path utilities for bwflow.
...
def get_blueprint_dir(repo_root: Path | None = None) -> Path:
    ...
    if repo_root is None:
        repo_root = get_repo_root()
    return repo_root / DIR_WORKFLOW / DIR_BLUEPRINT


def get_blueprint_readme(repo_root: Path | None = None) -> Path:
    """Get root blueprint README path.

    Args:
        repo_root: Repository root path. Defaults to auto-detected.

    Returns:
        Path to blueprint/README.md.
    """
    if repo_root is None:
        repo_root = get_repo_root()
    return repo_root / DIR_WORKFLOW / DIR_BLUEPRINT / "README.md"
...
if __name__ == "__main__":
    ...
    print(f"Blueprint dir: {get_blueprint_dir(repo)}")
