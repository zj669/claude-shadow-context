...
Triggered from `/bwflow:start` when the user describes a development task, especially when:
...
| Command | When to Use |
|---------|-------------|
| `/bwflow:start` | Entry point that triggers brainstorm |
| `/bwflow:finish-work` | After implementation is complete |
| `/bwflow:update-spec` | If new patterns emerge during work |
